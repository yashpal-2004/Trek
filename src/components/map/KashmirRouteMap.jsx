...
import { Landmark, Home, MapPin, Route, Info } from "lucide-react";
import Container from "../layout/Container";
import { routeWaypoints, routeStats, typeConfig } from "../../data/kashmir/routeMap";
...
export default function KashmirRouteMap() {
  ...
  const maxAltitude = Math.max(...altitudes, 2800);
  ...
  return (
    <section id="routemap" className="border-b border-black/8 py-16 scroll-mt-16 bg-white overflow-hidden">
      ...
      <div className="pb-12 border-b border-black/5 bg-[#FCFAF6]">
        <Container>
          <div className="flex items-center gap-2 text-teal-600 font-bold text-xs uppercase tracking-widest mb-2.5">
            <Route size={14} />
            <span>Interactive Route Map & Elevation Profile</span>
          </div>
          <h2 className="text-3xl font-black tracking-tight mb-3">KASHMIR VALLEY WANDERER ROUTE</h2>
          <p className="text-xs text-slate-500 max-w-xl">
            Track your journey from the plains of Delhi (210m) up to Srinagar (1,585m) and the towering alpine meadows of Gulmarg (2,650m) and Pahalgam (2,200m).
          </p>
          ...
          <div className="flex flex-wrap gap-6 mt-6">
            {[
              ...
              { label: "Transit Type",  value: "Train + Cabs + DEMU" },
              { label: "Region",        value: "Kashmir Valley, J&K" },
              { label: "Valley Towns",   value: "Gulmarg & Pahalgam" },
            ].map(s => (
              <div key={s.label}>
          ...
          <svg viewBox="0 0 1200 340" xmlns="http://www.w3.org/2000/svg" className="w-full" style={{ height: "240px" }}>
            <defs>
              <linearGradient id="ksGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#0D766E" stopOpacity="0.25" />
                <stop offset="100%" stopColor="#0D766E" stopOpacity="0.02" />
              </linearGradient>
            </defs>

            {/* Grid Lines */}
            {[210, 1000, 1585, 2200, 2650].map((alt) => {
              ...
              return (
                <g key={w.id} className="cursor-pointer" onClick={() => setActiveWp(w)}>
                  <circle
                    ...
                    cy={y}
                    r={isActive ? 6 : 4}
                    fill={w.highlight ? "#0D766E" : "#0D766E"}
                  ...
                  <text
                    ...
                    y={y - 12}
                    textAnchor="middle"
                    className={`text-[9px] font-bold ${isActive ? "fill-teal-600 font-extrabold" : "fill-slate-500"} transition-all`}
                  >
                    {w.name}
                  </text>
                </g>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-start text-left p-3 rounded-2xl border transition-all ${
                      isActive
                        ? "bg-teal-50 border-teal-200/50 shadow-sm"
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <p className="text-xs text-slate-400 font-mono">Distance from Delhi</p>

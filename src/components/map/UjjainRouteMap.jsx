...
import { Landmark, Home, MapPin, Route, Star, Info, Bus } from "lucide-react";
import Container from "../layout/Container";
import { routeWaypoints, routeStats, typeConfig } from "../../data/ujjain/routeMap";
...
const DISPLAY_CATS = ["All", "Start / End", "Hotel Stay", "Temple", "River Ghat"];
...
export default function UjjainRouteMap() {
  ...
  const maxAltitude = Math.max(...altitudes, 600);
  ...
  const [activeWp, setActiveWp] = useState(routeWaypoints[1]);
  ...
  return (
    <section id="routemap" className="border-b border-black/8 py-16 scroll-mt-16 bg-white overflow-hidden">
      ...
      <div className="pb-12 border-b border-black/5 bg-[#FCFAF6]">
        <Container>
          <div className="flex items-center gap-2 text-orange-600 font-bold text-xs uppercase tracking-widest mb-2.5">
            <Route size={14} />
            <span>Interactive Route Map</span>
          </div>
          <h2 className="text-3xl font-black tracking-tight mb-3">UJJAIN ROAD TRIP LAYOUT</h2>
          <p className="text-xs text-slate-500 max-w-xl">
            A spiritual weekend road trip starting from Sonipat, traveling by overnight AC sleeper bus to the ancient city of Ujjain, located on the Malwa Plateau.
          </p>
          ...
          <div className="flex flex-wrap gap-6 mt-6">
            {[
              ...
              { label: "Altitude", value: routeStats.peakAltitude },
              { label: "Duration",      value: "3 Days" },
              { label: "Transit Mode",  value: "AC Sleeper Bus" },
              { label: "Region",        value: "Malwa, Madhya Pradesh" },
              { label: "Key Temples",     value: `${routeStats.temples} Sites` },
            ].map(s => (
              <div key={s.label}>
                <p className="text-[9px] font-mono uppercase text-slate-400 tracking-widest">{s.label}</p>
                <p className="text-sm font-black text-orange-600">{s.value}</p>
              </div>
            ))}
          </div>
        </Container>
              ...
              return (
                <g key={w.id} className="cursor-pointer" onClick={() => setActiveWp(w)}>
                  <circle
                    ...
                    cy={y}
                    r={isActive ? 6 : 4}
                    fill={w.highlight ? "#EA580C" : "#F97316"}
                  ...
                  <text
                    ...
                    y={y - 12}
                    textAnchor="middle"
                    className={`text-[9px] font-bold ${isActive ? "fill-orange-600 font-extrabold" : "fill-slate-500"} transition-all`}
                  >
                    {w.name}
                  </text>
                </g>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                ...
                return (
                  ...
                  >
                    <div className={`p-2 rounded-xl mr-3 ${
                      isActive ? "bg-orange-100 text-orange-600" : "bg-slate-50 text-slate-400"
                    }`}>
                      <WpIcon size={16} />
                    </div>
                    <div>
          ...
          <div className="lg:col-span-2 flex flex-col justify-between bg-[#FCFAF6] border border-black/5 rounded-3xl p-6">
            <AnimatePresence mode="wait">
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    <span className="text-[10px] font-mono text-orange-600 bg-orange-50 border border-orange-100 px-2 py-0.5 rounded-full">
            ...
            <div className="flex items-center gap-2 mt-6 pt-4 border-t border-black/5 text-[11px] text-slate-400">
              ...
              <span>Select any waypoint on the elevation graph or the list to view trip details and stops.</span>
            </div>
          </div>
        </div>
      </Container>

...
import { routeWaypoints, routeStats, typeConfig } from "../../data/sikkim/routeMap";

const LUCIDE_MAP = {
  city: Home, junction: Route, town: Home, village: Home,
  valley: Mountain, lake: Waves, landmark: MapPin
};

const DISPLAY_CATS = ["All", "Sacred Lake", "Alpine Meadow", "Base Village", "Capital City", "Border Point", "Transit Stop"];

// SVG coordinate helpers
// Alt range: 115m (NJP) → 5430m (Gurudongmar Lake) | range = 5315m
// x: 80 → 1140 (1060px) over 1920 km total
// y: 310 → 30 (280px)
const MIN_ALT = 115;
const MAX_ALT = 5430;
...
// Linear sequence: Delhi → NJP → Gangtok → Lachen → Thangu → Gurudongmar → return
const MAIN_ROUTE_IDS = [1, 2, 3, 4, 5, 6, 10, 11];
// Lachung & Yumthang branch out from Gangtok/Lachen loop
const YUMTHANG_BRANCH_IDS = [4, 7, 8, 9];
...
const YUMTHANG_PATH = makePathD(YUMTHANG_BRANCH_IDS);
...
const LABEL_POINTS = [
  ...
  { id: 3, label: "Gangtok\n1,650m" },
  { id: 4, label: "Lachen\n2,750m" },
  { id: 6, label: "Gurudongmar\n5,430m" },
  { id: 8, label: "Yumthang\n3,560m" },
  { id: 11, label: "Delhi\n(Return)" }
];

const altLines = [115, 1500, 3000, 4500, 5430];

export default function SikkimRouteMap() {
      ...
      <div className="bg-[#F5F0E8] py-10 border-b border-black/8">
        <Container>
          ...
          <h2 className="text-3xl font-black uppercase tracking-tight mb-2 text-black" style={{ fontFamily: "'Anton', sans-serif" }}>
            North Sikkim Expedition Route
          </h2>
          <p className="text-xs text-slate-500 max-w-xl">
            7-day high altitude adventure loop starting from Delhi via NJP and Gangtok. Ascending to the sacred Gurudongmar Lake and exploring the flora-rich Yumthang Valley.
          </p>
          ...
          <div className="flex flex-wrap gap-6 mt-6">
            {[
              ...
              { label: "Border Near",    value: routeStats.borderProximity },
      ...
      <div className="bg-[#F5F0E8] overflow-x-auto border-b border-black/8">
        <div className="min-w-[700px]">
          <svg viewBox="0 0 1200 360" xmlns="http://www.w3.org/2000/svg" className="w-full" style={{ height: "280px" }}>
            <defs>
              <linearGradient id="sikkimGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#059669" stopOpacity="0.22" />
                <stop offset="100%" stopColor="#059669" stopOpacity="0.03" />
              </linearGradient>
              <filter id="sikkimGlow">
              ...
              return (
                <g key={alt}>
                  ...
                  <text x="72" y={y + 4} textAnchor="end"
                    ...
                    {alt === 115 ? "115m" : alt === 5430 ? "5,430m" : `${(alt / 1000).toFixed(0)}K`}
                  </text>
                </g>
              );
            })}
            ...
            {/* Fill under route */}
            <path d={FILL_PATH} fill="url(#sikkimGrad)" />

            {/* Yumthang branch line */}
            <path d={YUMTHANG_PATH} fill="none" stroke="#8B5CF6" strokeWidth="2"
              strokeDasharray="5 4" strokeLinejoin="round" strokeOpacity="0.8" />

            {/* Main route line */}
            <path d={MAIN_PATH} fill="none" stroke="#059669" strokeWidth="2.5"
              strokeLinejoin="round" strokeLinecap="round" filter="url(#sikkimGlow)" />
            ...
            {/* Yumthang branch dots */}
            {YUMTHANG_BRANCH_IDS.slice(1).map(id => {
              const w = getWpById(id);
              const { x, y } = toSVG(w.distance, w.altitude);
            ...
            {LABEL_POINTS.map(({ id, label }) => {
              ...
              const above = y < 180;
              const lx = id === 11 ? Math.min(x, 1125) : x;
              return (
                <g key={id}>
            ...
            <g transform="translate(88, 30)">
              <rect width="250" height="28" rx="6" fill="white" fillOpacity="0.7" />
              {[
                { color: "#059669", label: "Gurudongmar Loop", dash: false },
                { color: "#8B5CF6", label: "Yumthang Valley spur", dash: true  },
              ].map(({ color, label, dash }, i) => (
                <g key={label} transform={`translate(${12 + i * 115}, 14)`}>
            ...
            {/* Distance axis */}
            {[0, 400, 800, 1200, 1600, 1920].map(d => {

...
import { routeWaypoints, routeStats, typeConfig } from "../../data/hemkund/routeMap";
...
const DISPLAY_CATS = ["All", "Sacred Site", "Base Camp", "Valley", "Lake", "Trailhead", "Landmark", "Town / Hub", "Village"];

// SVG coordinate helpers
// Altitude range: 215m (Delhi) → 4632m (Hemkund) | Range = 4417m
// x: 80 → 1140 (1060px) over 505 km total road journey
...
// Route order for the main altitude profile (day-by-day sequence)
// Day 1: Delhi → Haridwar → (road) → Govindghat
// Day 2: Govindghat → Pulna → Ghangaria
...
// Day 6: Govindghat → Delhi (road)
const MAIN_ROUTE_IDS   = [1, 2, 4, 5, 6, 14, 15];     // Road + base route
const VALLEY_BRANCH_IDS = [6, 7, 8, 9];                // Valley of Flowers branch
const HEMKUND_BRANCH_IDS = [6, 10, 11, 12, 13];        // Hemkund Sahib branch
...
const VALLEY_PATH = makePathD(VALLEY_BRANCH_IDS);
const HEMKUND_PATH = makePathD(HEMKUND_BRANCH_IDS);
...
const LABEL_POINTS = [
  { id: 1,  label: "Delhi" },
  { id: 4,  label: "Govindghat" },
  { id: 6,  label: "Ghangaria\n3,048m" },
  { id: 8,  label: "Valley of Flowers\n3,658m" },
  { id: 12, label: "Hemkund Sahib\n4,632m" },
  { id: 15, label: "Delhi\n(Return)" }
];

const altLines = [215, 1000, 2000, 3000, 4000, 4632];

export default function HemkundRouteMap() {
      ...
      <div className="bg-[#F5F0E8] py-10 border-b border-black/8">
        <Container>
          ...
          <p className="text-xs text-slate-500 max-w-xl">
            6-day round trek from Delhi — two UNESCO/sacred alpine destinations accessed from Ghangaria base camp.
          </p>
          ...
          <div className="flex flex-wrap gap-6 mt-6">
            {[
              ...
              { label: "UNESCO Site",   value: routeStats.unescoSite },
      ...
      <div className="bg-[#F5F0E8] overflow-x-auto border-b border-black/8">
        <div className="min-w-[700px]">
          <svg viewBox="0 0 1200 360" xmlns="http://www.w3.org/2000/svg" className="w-full" style={{ height: "280px" }}>
            <defs>
              <linearGradient id="hemkGrad" x1="0" y1="0" x2="0" y2="1">
              ...
              return (
                <g key={alt}>
                  ...
                  <text x="72" y={y + 4} textAnchor="end"
                    ...
                    {alt === 215 ? "215m" : alt === 4632 ? "4,632m" : `${(alt / 1000).toFixed(0)}K`}
                  </text>
                </g>
              );
            })}
            ...
            <path d={FILL_PATH} fill="url(#hemkGrad)" />

            {/* Valley of Flowers branch fill */}
            {(() => {
              const vPts = VALLEY_BRANCH_IDS.map(id => {
                const w = getWpById(id);
                return toSVG(w.distance, w.altitude);
              });
              const vLast = vPts[vPts.length - 1];
              const vFirst = vPts[0];
              const vFill = VALLEY_PATH + ` L ${vLast.x.toFixed(1)},310 L ${vFirst.x.toFixed(1)},310 Z`;
              return <path d={vFill} fill="url(#valleyGrad)" />;
            })()}

            {/* Valley of Flowers branch line */}
            <path d={VALLEY_PATH} fill="none" stroke="#65A30D" strokeWidth="2"
              strokeDasharray="5 4" strokeLinejoin="round" strokeOpacity="0.8" />

            {/* Hemkund branch line */}
            <path d={HEMKUND_PATH} fill="none" stroke="#8B5CF6" strokeWidth="2"
              strokeDasharray="5 4" strokeLinejoin="round" strokeOpacity="0.8" />

            {/* Main route line */}
            <path d={MAIN_PATH} fill="none" stroke="#0D9488" strokeWidth="2.5"
              strokeLinejoin="round" strokeLinecap="round" filter="url(#hemkGlow)" />
            ...
            {/* Valley branch dots */}
            {VALLEY_BRANCH_IDS.slice(1).map(id => {
              const w = getWpById(id);
              const { x, y } = toSVG(w.distance, w.altitude);
              return <circle key={id} cx={x} cy={y} r="2.5" fill="#65A30D" stroke="white" strokeWidth="1" opacity="0.9" />;
            })}

            {/* Hemkund branch dots */}
            {HEMKUND_BRANCH_IDS.slice(1).map(id => {
              const w = getWpById(id);
              const { x, y } = toSVG(w.distance, w.altitude);
            ...
            {LABEL_POINTS.map(({ id, label }) => {
              ...
              const above = y < 180;
              const lx = id === 15 ? Math.min(x, 1130) : x;
              return (
                <g key={id}>
            ...
            <g transform="translate(88, 22)">
              <rect width="310" height="28" rx="6" fill="white" fillOpacity="0.7" />
              {[
                { color: "#0D9488", label: "Main Route", dash: false },
                { color: "#65A30D", label: "Valley of Flowers", dash: true },
                { color: "#8B5CF6", label: "Hemkund Sahib", dash: true },
              ].map(({ color, label, dash }, i) => (
                <g key={label} transform={`translate(${12 + i * 100}, 14)`}>
